...
sourcename = "Assignment.java"
nativeName = "org/js/JsSystem.java"
...
$(document).ready ->
  ...
  waitmsg = "Running your code ... Please wait ...."
  
  runresult = ((str) -> 
             cur = $('#mystdout').val()
             if cur == waitmsg 
                cur = ""
             $('#mystdout').val(cur+str))
             # alert(str))
  ...
  $('#save_btn').click (e) ->
    ...
    if testfilename != ""
        ...
        node.fs.writeFileSync(testfilename, testcode)
    classname = sourcename.split('.')[0]
    $('#mystdout').val(waitmsg)
    contents = editor.getSession().getValue()
    contents += '\n' unless contents[contents.length-1] == '\n'
    node.fs.writeFileSync(sourcename, contents)
    jvm.set_classpath '/home/doppio/vendor/classes/', './:/home/doppio'
    rs = new runtime.RuntimeState(runresult, user_input, bs_cl)
    jvm.run_class(rs, 'classes/util/Javac', [sourcename], (->
      commands.clear_cache()
      #controller.message("File saved as '#{fname}', compiled and run.", 'success')
      if $('#mystdout').val() == waitmsg 
         commands.java([classname])
    ))
#      close_editor()
    e.preventDefault()
...
commands =
  android: ->
    node.fs.writeFileSync(nativeName, jsFileContent) 
    $('#mystdout').val("")
    # commands.edit([sourcename])
    args = [sourcename]
    try
  ...
  javac: (args, cb) ->
    ...
    #controller.message("compiled.", 'success')
...
jsFileContent =
  ...
  public class JsSystem {
    public static native void javascriptAlert(String message);
    public static native void exec(String message);
  }
  """
